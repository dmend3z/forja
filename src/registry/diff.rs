...
fn parse_diff_output(output: &str) -> Result<Vec<SkillChange>> {
    // Group file changes by skill directory (skills/<phase>/<tech>/<name>/)
    ...
    for line in output.lines() {
        ...
        // Extract skill ID from path: skills/<phase>/<tech>/<name>/...
        ...
        // Merge: if any file is added and another modified, prefer the more significant change
        skill_changes
            .entry(skill_id)
            .and_modify(|existing| {
                // Added + anything = Added (new skill)
                // Removed + anything = depends, but Modified is safer
...
mod tests {
    ...
    fn parse_deleted_file() {
        let output = "D\tskills/test/tdd/workflow/skill.json\n";
        let changes = parse_diff_output(output).unwrap();
        assert_eq!(changes.len(), 1);
        assert_eq!(changes[0].skill_id, "test/tdd/workflow");
        assert_eq!(changes[0].change_type, ChangeType::Removed);
    }

    #[test]
    fn parse_multiple_files_same_skill() {
        let output = "M\tskills/code/rust/coder/skill.json\nM\tskills/code/rust/coder/agents/coder.md\n";
        let changes = parse_diff_output(output).unwrap();
        assert_eq!(changes.len(), 1);
        assert_eq!(changes[0].skill_id, "code/rust/coder");
    }
    ...
    #[test]
    fn parse_non_skill_files_ignored() {
